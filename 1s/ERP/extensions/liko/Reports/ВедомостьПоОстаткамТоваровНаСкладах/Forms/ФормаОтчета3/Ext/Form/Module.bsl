...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	//+ Лико m.shenderov 15.07.2019
	Структура=ОбработкаРасшифорвки(Расшифровка);
Если ЗначениеЗаполнено(Структура.Номенклатура) Тогда	
	СтандартнаяОбработка=Ложь;	
	ОписаниеОповещение= Новый  ОписаниеОповещения("ВыборДействияЗавершение",ЭтаФорма,Структура); 
	СписокЗначений= Новый СписокЗначений;
	СписокЗначений.Добавить("Установка миниального (min) запаса");
	ОбработкаРасшифровка= Новый ОбработкаРасшифровкиКомпоновкиДанных(ОтчетДанныеРасшифровки, Новый ИсточникДоступныхНастроекКомпоновкиДанных(Отчет));	
	ОбработкаРасшифровка.ПоказатьВыборДействия(ОписаниеОповещение,Расшифровка,,СписокЗначений,,);
КонецЕсли;
//- Лико m.shenderov 15.07.2019
...
&НаСервере
Функция  ОбработкаРасшифорвки(Расшифровка) //+Лико m.shenderov 13.04.2018
	...
	Если ТипЗнч(Расшифровка) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных") тогда
		...
		Если ТипЗнч(ЭлементРасшифровки) = Тип("ЭлементРасшифровкиКомпоновкиДанныхПоля") Тогда			
			...
			Структура.Вставить("Группировка",Справочники.ГруппировкиТоварныхОграничений.ПустаяСсылка());
			
			
			Для каждого Поле Из ЭлементРасшифровки.ПолучитьПоля() Цикл
				Если Поле.Поле="Номенклатура" Тогда
					Структура.Вставить("Номенклатура",Поле.Значение);
				ИначеЕсли Поле.Поле="Характеристика" Тогда
					Структура.Вставить("Характеристика",Поле.Значение);  					
				КонецЕсли;
...
КонецФункции  //-Лико m.shenderov 13.04.2018


&НаКлиенте
Процедура ВыборДействияЗавершение(ВыполненноеДействие,ПараметрВыполненногоДействия,ДополнительныеПараметры) Экспорт //+Лико m.shenderov 16.04.2018
	
	Если ЗначениеЗаполнено(ДополнительныеПараметры)И ВыполненноеДействие="Установка миниального (min) запаса" Тогда
		Если НЕ ЕстьКлюч(ДополнительныеПараметры) Тогда
			ОткрытьФорму("РегистрСведений.ТоварныеОграничения.ФормаЗаписи",Новый Структура("ЗначенияЗаполнения", Новый Структура("Склад,Номенклатура,Характеристика,Группировка,ЭтоДействующийПараметр,ОбеспечениеЗаказовПриПоддержанииЗапаса",
			ДополнительныеПараметры.Склад,ДополнительныеПараметры.Номенклатура,ДополнительныеПараметры.Характеристика,ДополнительныеПараметры.Группировка,Истина,ПредопределенноеЗначение("Перечисление.ОбеспечениеЗаказовПриПоддержанииЗапаса.ЗаСчетЗапасов"))));
		Иначе
			
			
			ПараметрыМассив=Новый Массив;
			ПараметрыМассив.Добавить(ДополнительныеПараметры);
			Ключ = Новый("РегистрСведенийКлючЗаписи.ТоварныеОграничения", ПараметрыМассив); 
			ОткрытьФорму("РегистрСведений.ТоварныеОграничения.ФормаЗаписи",
...
КонецПроцедуры //-Лико m.shenderov 16.04.2018

&НаСервере
Функция ЕстьКлюч(ДополнительныеПараметры) //+Лико m.shenderov 16.04.2018  
	...
	|	И ТоварныеОграничения.Группировка = &Группировка";
	...
	Запрос.УстановитьПараметр("Группировка",	ДополнительныеПараметры.Группировка );  			
...
КонецФункции  //-Лико m.shenderov 16.04.2018
