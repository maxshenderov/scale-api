...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтобразитьСостояниеОтчета(НСтр("ru = 'Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';
									|en = 'Report not generated. To generate the report, click ""Run report"".'"));
	...
	КонецЕсли;
	//+ Лико m.shenderov 03/12/2019
	Команда =Команды.Добавить("Лико_ОтправитьСебе");
	Команда.Заголовок = "Отправить себе";
	Команда.Действие = "Лико_ОтправитьСебеПосле";
	Команда.Картинка=Команды.Найти("ОтправитьПоПочте").Картинка;
	Команда.Отображение=ОтображениеКнопки.КартинкаИТекст;
	Элемент = Элементы.Добавить("Лико_ОтправитьСебе", Тип("КнопкаФормы"), Элементы.ГруппаОтправить);
	Элемент.ИмяКоманды="Лико_ОтправитьСебе";
	Элементы.Переместить(Элемент,Элементы.ГруппаОтправить,Элементы.ОтправитьПоПочте);
	Элементы.Переместить(Элементы.ОтправитьПоПочте,Элементы.ГруппаОтправить,Элемент);
	//+ Лико m.shenderov 18.12.2017	
	Если ВидимостьКомандВариантовОтчетов = Ложь Тогда
		НастройкиОтчета.РазрешеноИзменятьВарианты = Ложь;
		НастройкиОтчета.РазрешеноВыбиратьВарианты = Ложь;
		Если ПустаяСтрока(КлючНазначенияИспользования) Тогда
...
Процедура ПослеФормирования(Результат, ЗагрузитьРезультатФормированияОтчета) Экспорт 
	Если Результат = Неопределено Тогда 
		ПоказатьОшибкиФормирования(НСтр("ru = 'Формирование отчета прервано администратором';
										|en = 'The report generation is canceled by administrator.'"));
...
Процедура ЗагрузитьРезультатФормированияОтчета()
	...
	Если Результат = Неопределено Тогда 
		ПоказатьОшибкиФормирования(НСтр("ru = 'Не удалось сформировать отчет (пустой результат)';
										|en = 'Cannot generate the report (the result is blank).'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьВариант(КлючВарианта)
	...
	ОтобразитьСостояниеОтчета(НСтр("ru = 'Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';
									|en = 'Another report option is selected. To generate report, Click ""Run report"".'"),
	...
	Если ПараметрыОбновления.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли ПараметрыОбновления.ВариантМодифицирован
			...
			ОтобразитьСостояниеОтчета(НСтр("ru = 'Изменились настройки. Нажмите ""Сформировать"" для получения отчета.';
											|en = 'Settings changed. To generate report, click ""Run report"".'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура Лико_ОтправитьСебеПосле(Команда)
	...
	ПараметрыОтправки = МодульРаботаСПочтовымиСообщениямиКлиент.ПараметрыОтправкиПисьма();
	ПараметрыОтправки.Тема ="Отчет: "+ЭтаФорма.Заголовок;
	ПараметрыОтправки.Вложения = СписокВложений;
...
Процедура ОбработатьНоменклатуру(Результат) //+Лико m.shenderov 08.12.2017
	...
	//	Если Элемент.Заголовок="Объект номенклатуры" И
	//		Элемент.Вид=ВидПоляФормы.ПолеВвода Тогда
	Для каждого Отбор Из  Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		Если Отбор.ЛевоеЗначение= Новый ПолеКомпоновкиДанных("ОбъектНоменклатуры") Тогда
			ОтборОбъектНоменклатуры=Отбор;
			ОтборОбъектНоменклатуры=Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ОтборОбъектНоменклатуры.ИдентификаторПользовательскойНастройки);
			ОтборОбъектНоменклатуры.ПравоеЗначение= Результат.ОбъектНоменклатуры;
			ОтборОбъектНоменклатуры.Использование=Истина;
		КонецЕсли;				
	КонецЦикла; 			
	//	КонецЕсли;	
	//КонецЦикла;
	...
	//			ПоказатьВводЧисла(ОписаниеОповещения,,"Ввидите количество:",15,3); 			
	//		КонецЕсли; 
	//		
	//	КонецЕсли;
	//	
