...
Процедура ВыбратьЗначениеНоменклатуры(Команда)
	...
	Массив = Новый Массив();	
	Для каждого СтрокаВыбора Из МассивВыделенныхСтрок Цикл
		Элементы.СерииНоменклатуры.ТекущаяСтрока=СтрокаВыбора;
		Товар= Элементы.СерииНоменклатуры.ТекущиеДанные;
		...
		Структура.Вставить("Ячейка",Неопределено);
		ЗаполнитьЗначенияСвойств(Структура,Товар);
		Структура.КоличествоУпаковок=Товар.Остаток;		
...
Процедура УстановитьВидимость() //+Лико m.shenderov 27.10.2017
	...
	СерииНоменклатуры.Параметры.УстановитьЗначениеПараметра("Склад",Склад);
	СерииНоменклатуры.Параметры.УстановитьЗначениеПараметра("Помещение",Помещение);
	СписокНоменклатура.Параметры.УстановитьЗначениеПараметра("Склад",Склад);
	СписокНоменклатура.Параметры.УстановитьЗначениеПараметра("Помещение",Помещение);
...
Структура = Новый Структура;
Структура.Вставить("Ключ",Элементы.СерииНоменклатуры.ТекущаяСтрока);
ОткрытьФорму("Справочник.СерииНоменклатуры.ФормаОбъекта",Структура,ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
