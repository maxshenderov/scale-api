...
Процедура ВыполнитьЗапросЗаполненияТаблицыОстатков()
	...
	// --> Начало Лико Ефимов ЕВВ 12.05.2017
	//Если выбран тип "ВсеСерии", то будем выводить ВСЕ зарегистрированные серии для характеристики и вида номенклатуры (а не только те, по которым когда-то были движения)
	//Запрос взят из одноименной процедуры формы "РегистрацияИПодборСерийПоОднойСтрокеТоваров" (+ убрано из него количество и количество упаковок)
	Если Запрос.Параметры.ВсеСерии Тогда
		Запрос.УстановитьПараметр("ВидНоменклатуры", Запрос.Параметры.Номенклатура.ВидНоменклатуры);
		...
		Запрос.Текст = Запрос.Текст +
			...
			|		И НЕ Серии.ПометкаУдаления) КАК ВложенныйЗапрос
			|
			|СГРУППИРОВАТЬ ПО
			...
			|	НомерКиЗГИСМ";
	КонецЕсли;
	// <-- Конец Лико Ефимов ЕВВ 12.05.2017
