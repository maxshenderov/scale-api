...
//
// Параметры:
//  Показатели   - см. ЗаполнениеФормСтатистики.НовыйОписаниеПоказателей
//
...
// Выполняет действия перед расчетом показателей.
//
// Параметры:
//  Показатели   - см. ЗаполнениеФормСтатистики.НовыйОписаниеПоказателей
//  ПараметрыОтчета - Структура - структура параметров отчета.
//  ТаблицаРасшифровки - ТаблицаЗначений - передача даннных алгоритма между разными его частями.
//                       См. ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьСтрокуРасшифровки.
//
...
// Выполняет действия после расчета показателей.
//
// Параметры:
//  Показатели   - см. ЗаполнениеФормСтатистики.НовыйОписаниеПоказателей
//  ТаблицаРасшифровки - ТаблицаЗначений - передача даннных алгоритма между разными его частями.
...
Функция КлючевыеПоля(Алгоритм) Экспорт
	...
	ИначеЕсли Алгоритм = ЧисленностьПерсонала Тогда
		Возврат "НачалоПериода, КонецПериода, ДополнительныйАлгоритм";
	ИначеЕсли Алгоритм = ВыручкаПредпринимателя Тогда
		Возврат "НачалоПериода, КонецПериода, Организация, ДополнительныйАлгоритм";
	Иначе
...
// Выполняет действие с показателем.
//
// Параметры:
//  Показатели   - ТаблицаЗначений - данные, которые нужно рассчитать. См. ЗаполнениеФормСтатистики.НовыйОписаниеПоказателей.
//  Формулы      - см. НайтиФормулы.
//  ТаблицаРасшифровки - ТаблицаЗначений - передача даннных алгоритма между разными его частями.
//                       См. ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьСтрокуРасшифровки.
//
Процедура ДобавитьВычестьОтборПоказателя(Показатели, Формулы, ТаблицаРасшифровки)
	...
	Для Каждого Формула Из Формулы Цикл
		...
		НастройкаВXML = Формула.Настройка;
		ОтборДляДобавленияНастройки = ЗаполнениеФормСтатистики.ПолучитьОтборКомпоновкиДанных(Формула.Настройка);
		
		// Если отбор берется из ненастраиваемого объекта наблюдения, то настройка отбора считана в ОбщаяНастройка
		Если ОтборДляДобавленияНастройки = Неопределено И ЗначениеЗаполнено(Формула.ОбъектНаблюдения)
			 И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Формула.ОбъектНаблюдения, "ТребуетНастройки") Тогда
			НастройкаВXML = Формула.ОбщаяНастройка;
			ОтборДляДобавленияНастройки = ЗаполнениеФормСтатистики.ПолучитьОтборКомпоновкиДанных(Формула.ОбщаяНастройка);
		КонецЕсли;
		...
		Если Формула.ДополнительныйАлгоритм = ДобавитьОтборПоказателя Тогда
			ДополнительныеПараметры.Отбор = НастройкаВXML;
		ИначеЕсли Формула.ДополнительныйАлгоритм = ВычестьОтборПоказателя Тогда
			Отбор = Новый ОтборКомпоновкиДанных;
			ГруппаОтбора = Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
			ГруппаОтбора.Использование = Истина;
