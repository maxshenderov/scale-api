...
### 📝 Правило: реестр задач + state.md (обязательно)

**При старте сессии (первое действие):**
1. Прочитай `output/session/tasks.md`
2. Покажи список активных задач пользователю
3. Спроси: продолжить задачу (номер), создать новую, или закрыть задачу
4. После выбора — обнови `tasks.md`

**Управление задачами:**
- **Новая задача:** добавь запись в `## Активные` в `tasks.md`, создай `output/session/state-<slug>.md`
- **Закрыть задачу:** перенеси запись в `## Закрытые` в `tasks.md`
- **Возобновить:** перенеси из `## Закрытые` в `## Активные`

Формат записи в `tasks.md`:
```
- **NN** | slug-задачи | ветка | YYYY-MM-DD
```

**После каждого значимого шага** — обнови `output/session/state-<slug>.md`:
- **Текущий шаг** — что делаем прямо сейчас
- **Что сделано** — галочки + хэши коммитов
- **Затронутые файлы**
- **Известные проблемы**

Это делаешь ТЫ, не хук. Хук только коммитит.

### 🚫 Git — запрет

НИКОГДА не делай `git reset --hard`, `git checkout -- .`, `git clean -fd`
без явного разрешения пользователя. При проблеме — `git stash`, не откат.
Каждый ход коммитится и пушится хуком автоматически — труд не теряется.
