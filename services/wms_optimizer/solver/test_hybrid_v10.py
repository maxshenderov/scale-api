"""Тест Hybrid V10 — V3 холодный старт + CP-SAT section-level реслот + валидация."""
import json
import os
import sys
import time
...
from solver.hybrid_v10 import run_hybrid_v10
...
def main():
    ...
    # =====================================================================
    # ПРОХОД 1: внутри V10 (холодный старт + реслот)
    # =====================================================================
    ...
    req = OptimizationRequest(
        optimizationId="v10-test", mode="place",
        occupancy=occ, newPallets=floor, settings=settings,
    )

    print(f"\n{'='*60}")
    print(f"Hybrid V10: V3 cold start + CP-SAT reslot")
    print(f"{'='*60}")

    t0 = time.time()
    resp = run_hybrid_v10(req)
    ...
    print(f"ИТОГО")
    print(f"{'='*60}")
    print(f"  V10:              {placed}/{total_pallets} ({placed/total_pallets*100:.1f}%) — {elapsed:.1f}с, {err_count} ошибок")
    print(f"  V3 baseline:      3215/3406 (94.4%) — 4.0с, 0 ошибок")
    print(f"  V5 baseline:      3236/3406 (95.0%) — 368с, 0 ошибок")
    print(f"  V6 cold:          3215/3406 (94.4%) — 8-16с, 0 ошибок")
