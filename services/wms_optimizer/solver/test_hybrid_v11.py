"""Тест Hybrid V11 — V3 cold start + compaction + V3 re-placement + валидация."""
import json
import os
import sys
import time
...
from solver.hybrid_v11 import run_hybrid_v11
...
def main():
    ...
    req = OptimizationRequest(
        optimizationId="v11-test", mode="place",
        occupancy=occ, newPallets=floor, settings=settings,
    )

    print(f"\n{'='*60}")
    print(f"Hybrid V11: V3 cold start + Compaction + V3 re-placement")
    print(f"{'='*60}")

    t0 = time.time()
    resp = run_hybrid_v11(req)
    ...
    print(f"ИТОГО")
    print(f"{'='*60}")
    print(f"  V11:             {placed}/{total_pallets} ({placed/total_pallets*100:.1f}%) — {elapsed:.1f}с, {err_count} ошибок")
    print(f"  V3 baseline:     3215/3406 (94.4%) — 4.0с, 0 ошибок")
    print(f"  V10 (no reslot): 3215/3406 (94.4%) — 5.9с, 0 ошибок")
