"""Тест Hybrid V6 — двухпроходный: холодный старт → реслот + валидация."""
import json
import os
import sys
import time
...
from solver.hybrid_v6 import run_hybrid_v6
...
def build_occupancy_from_ops(occ, pallet_map, operations):
    ...
    for row in occ:
        ...
        # Очищаем ВСЕ паллетные поля перед заполнением
        for i in range(1, 4):
            for field in ("id", "code", "width", "height", "depth", "weight"):
                row_dict[f"pallet{i}_{field}"] = "" if field == "id" or field == "code" else 0
            row_dict[f"quantity{i}"] = 0

        # Заполняем паллетами из операций
...
def main():
    ...
    req1 = OptimizationRequest(
        optimizationId="v6-pass1", mode="place",
        occupancy=occ, newPallets=floor, settings=settings1,
    )

    t0 = time.time()
    resp1 = run_hybrid_v6(req1)
    ...
    if not_placed1 == 0:
        ...
        # Строим occupancy из ОПЕРАЦИЙ прохода 1 (правильные адреса!)
        ...
        if not leftovers:
            ...
            req2 = OptimizationRequest(
                optimizationId="v6-pass2", mode="place",
                occupancy=occ2, newPallets=leftovers, settings=settings2,
            )

            t0_2 = time.time()
            resp2 = run_hybrid_v6(req2)
