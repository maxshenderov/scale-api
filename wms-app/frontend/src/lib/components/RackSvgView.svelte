<script lang="ts">
    interface Props { racks: any[]; sections: any[]; selectedRack: string; onpalletdrop?: (palletId: string, targetAddressId: string) => void; }
    let { racks, sections, selectedRack, onpalletdrop }: Props = $props();

    const MM = 0.05; // МасштабМм из 1С
    let zs = $state(typeof window !== 'undefined' ? parseInt(localStorage.getItem('zs_scale') || '100') : 100);
    let tip = $state({ show: false, x: 0, y: 0, text: '' });

    function applyScale(v: number, save = true) { zs = Math.max(0, Math.min(1000, Math.round(v))); if (save && typeof window !== 'undefined') localStorage.setItem('zs_scale', String(zs)); }
    ...
    function dragPallet(e: DragEvent) { const g = (e.currentTarget as HTMLElement)?.getAttribute('data-pallet-guid'); if (!g) return; e.dataTransfer!.setData('text/plain', g); e.dataTransfer!.effectAllowed = 'move'; }
    ...
    function dropOnCell(e: DragEvent) { e.preventDefault(); (e.currentTarget as HTMLElement)?.classList.remove('drag-over'); const pg = e.dataTransfer!.getData('text/plain'); const cg = (e.currentTarget as HTMLElement)?.getAttribute('data-cell-guid'); if (pg && cg && onpalletdrop) onpalletdrop(pg, cg); }

    let activeRack = $derived(racks.find((r: any) => r.id === selectedRack));
    function floorSections(floorNum: number): any[] { return sections.filter((s: any) => (s.rack_id === activeRack?.id || s.rack === activeRack?.id) && s.floor === floorNum); }
    function palletAt(sec: any, idx: number): any | null { const code = sec?.[`pallet${idx + 1}_code`]; if (!code) return null; return { guid: sec[`pallet${idx + 1}_id`] || '', code, w: sec[`pallet${idx + 1}_width`] || 0, h: sec[`pallet${idx + 1}_height`] || 0, d: sec[`pallet${idx + 1}_depth`] || 0, weight: sec[`pallet${idx + 1}_weight`] || 0 }; }

    // Позиция паллета в PX внутри ячейки (из 1С: строки 2282-2294)
    function palletLeftPx(sec: any, addrIdx: number, secWPx: number): number {
        const p = palletAt(sec, addrIdx);
        const pWPx = p ? Math.round(p.w * MM * 10) / 10 : 0;
        const cellWPx = secWPx / 3;
        const nAddr = 3;
        if (nAddr === 1) return Math.round(((secWPx - pWPx) / 2) * 10) / 10;
        if (addrIdx === 0) return 0;
        if (addrIdx === nAddr - 1) return Math.round((cellWPx - pWPx) * 10) / 10;
        // Средний: центр секции минус смещение ячейки
        return Math.round(((secWPx - pWPx) / 2 - addrIdx * cellWPx) * 10) / 10;
    }

    // Перекрыта ли ячейка соседним паллетом (из 1С: строки 2141-2151)
    function isCellOverlapped(sec: any, cellIdx: number, secWMm: number): boolean {
        const nAddr = 3;
        const cellWMm = secWMm / nAddr;
        for (let ia = 0; ia < nAddr; ia++) {
            if (ia === cellIdx) continue;
            const p = palletAt(sec, ia);
            if (!p) continue;
            const pw = p.w;
            let leftMm: number;
            if (nAddr === 1) leftMm = (secWMm - pw) / 2;
            else if (ia === 0) leftMm = 0;
            else if (ia === nAddr - 1) leftMm = secWMm - pw;
            else leftMm = (secWMm - pw) / 2;
            if (leftMm < (cellIdx + 1) * cellWMm && (leftMm + pw) > cellIdx * cellWMm) return true;
        }
        return false;
    }

    function tipText(p: any): string { return 'П' + p.code + '\n' + p.h + '×' + p.w + '×' + p.d + '\n' + p.weight + ' кг'; }
</script>
...
{#if activeRack}
    <div class="flex flex-col flex-1 min-h-0">
        <div class="scale-bar">
            ...
            <div class="sc-slider-wrap"><input type="range" min="0" max="1000" value={zs} oninput={(e) => applyScale(parseInt(e.currentTarget.value))} /></div>
            ...
            <div id="sc" style="transform:scale({zs / 100});transform-origin:top left;">
                <div class="racking">
                    {#each [...(activeRack.floors || [])].sort((a: any, b: any) => b.number - a.number) as floor}
                        {@const ts = floor.typeSize || {width: 2700, height: 2100, weight: 0}}
                        {@const hPx = Math.round((ts.height || 2100) * MM * 10) / 10}
                        {@const wPx = Math.round((ts.width || 2700) * MM * 10) / 10}
                        {@const fs = floorSections(floor.number)}
                        ...
                        <div class="floor">
                            ...
                            <div class="floor-content">
                                <!-- Шапки секций -->
                                <div style="display:flex;align-items:stretch;margin-bottom:0"><div class="rack-post"></div>
                                    {#each Array(activeRack.sectionsCount || 17) as _, si}
                                        {@const sec = fs[si]}
                                        <div class="rack-post"></div>
                                        <div class="section-hdr" style="width:{wPx}px">
                                            <div class="section-hdr-name"><span>{sec?.section_code || '—'}</span></div>
                                            <div class="section-hdr-cells"><div>{sec?.address1 || ''}</div><div>{sec?.address2 || ''}</div><div>{sec?.address3 || ''}</div></div>
                                        </div>
                                    {/each}
                                    <div class="rack-post"></div>
                                </div>
                                ...
                                <!-- Ряд секций -->
                                <div class="sections-row"><div class="rack-post"></div>
                                    {#each Array(activeRack.sectionsCount || 17) as _, si}
                                        {@const sec = fs[si]}
                                        {@const hasPallet = sec && !!(sec.pallet1_code || sec.pallet2_code || sec.pallet3_code)}
                                        {@const restricted = sec?.restricted}
                                        {@const secCls = restricted ? 'section section-restricted' : (!hasPallet ? 'section section-empty' : 'section')}
                                        <div class={secCls} style="width:{wPx}px;height:{hPx}px">
                                            <div class="cells-container">
                                                {#each Array(3) as _, ai}
                                                    {@const pallet = sec ? palletAt(sec, ai) : null}
                                                    {@const overlapped = sec && !pallet ? isCellOverlapped(sec, ai, ts.width || 2700) : false}
                                                    {@const cellFree = sec && !pallet && !overlapped && !restricted}
                                                    {@const cellCls = pallet ? 'cell' : (overlapped || restricted ? 'cell' : 'cell cell-free')}
                                                    {@const cellGuid = sec?.[`address${ai + 1}`] || ''}
                                                    <div class={cellCls}
                                                         data-cell-guid={cellGuid}
                                                         ondragover={cellFree ? dragOverCell : undefined}
                                                         ondragleave={cellFree ? dragLeaveCell : undefined}
                                                         ondrop={cellFree ? dropOnCell : undefined}
                                                         role="button" tabindex={cellFree ? 0 : -1}>
                                                        {#if pallet}
                                                            {@const pWPx = Math.round(pallet.w * MM * 10) / 10}
                                                            {@const pHPx = Math.min(hPx, Math.round(pallet.h * MM * 10) / 10)}
                                                            {@const leftPx = palletLeftPx(sec!, ai, wPx)}
                                                            <div class="pallet pallet-occupied" draggable="true"
                                                                ...
                                                                <div class="pallet-label">П{pallet.code}</div>
                                                                <div class="pallet-size">{pallet.h}×{pallet.w}×{pallet.d}</div>
                                                            </div>
                                                        {/if}
                                                    </div>
                                                {/each}
    ...
    {#if tip.show}
        <div class="pallet-tooltip" style="display:block;left:{tip.x}px;top:{tip.y}px">
            {#each tip.text.split('\n') as line}{line}<br />{/each}</div>
    {/if}
{:else}
...
<style>
    ...
    .floor-label{font-weight:bold;font-size:11px;color:#000}
    ...
    .section-empty{background:rgba(200,230,200,.3)}
    .section-restricted{background:rgba(230,200,200,.35)}
    .section-restricted .cell{background:rgba(230,180,180,.45)}
    ...
    .pallet-tooltip{position:fixed;background:rgba(26,79,180,.94);color:#fff;font-size:9px;font-weight:bold;padding:4px 8px;white-space:pre-line;text-align:center;z-index:200;pointer-events:none;border-radius:8px;line-height:1.3;box-shadow:0 2px 8px rgba(0,0,0,.25)}
